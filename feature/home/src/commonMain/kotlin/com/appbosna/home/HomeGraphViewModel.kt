...
class HomeGraphViewModel(
    ...
    init {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            delay(2000) // ne blokira main thread
            println("ProductRepository initialized.")
        }
    }
        ...
        .flatMapLatest { data ->
            if (data.isSuccess()) {
                ...
                val totalPrice = cartItems.sumOf { cartItem ->
                    val productPrice = products[cartItem.productId]?.price?.toDouble() ?: 0.0
                    productPrice * cartItem.quantity.toIntSafe()
                }
